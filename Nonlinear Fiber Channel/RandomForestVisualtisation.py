...
p=[5,10,15,20,25,30,35,40,44,50]
SERforest=np.zeros((len(N),len(L),len(p)))
SERforestmit=np.zeros((len(N),len(L),len(p)))
best=[]
bestmit=[]
for l in range(len(L)):
    for n in range(len(N)):
        for i in range(len(p)):
            data = np.loadtxt('N%iL%ikmP%imW.txt'%(N[n],L[l],p[i]))
            datamit=[]
            ...
            plt.savefig('DataN%iL%ikmP%imW.png'%(N[n],L[l],p[i]),dpi=250)
            plt.close()
            ...
            visualize_classifier(forest,X_train,y_train,'ConstN%iL%ikmP%imW.png'%(N[n],L[l],p[i]))
            SERforest[n,l,i]=BER(y_test,y_prediction)
